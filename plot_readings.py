...
for timestamp in timestamps:
    ...
    ax.text(timestamp['time'], random.randint(240,320), timestamp['message'], fontsize=6)
...
# set axis scale
ax.axis([ 0, max(timestamps, key=lambda x: x['time'])['time'] + 1, 0, max(readings, key=lambda x: x['power'])['power'] + 10])
